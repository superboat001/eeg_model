...
EEG_NET_ENTRYPOINT="${PROJECT_ROOT}/base_model/run_eeg_net.py"

usage() {
    cat <<'EOF'
Usage:
  ./run_eeg_net_background.sh [start] [run_eeg_net.py options]
  ./run_eeg_net_background.sh sanity [run_eeg_net.py options]
  ./run_eeg_net_background.sh status
  ./run_eeg_net_background.sh tail [lines]
  ./run_eeg_net_background.sh help

Examples:
  ./run_eeg_net_background.sh
  ./run_eeg_net_background.sh start --device cuda:0 --run-name eeg_net_control
  ./run_eeg_net_background.sh start --split-seed-start 2030 --split-seed-count 10
  ./run_eeg_net_background.sh sanity --device cuda:0
  ./run_eeg_net_background.sh status
  ./run_eeg_net_background.sh tail 120

The EEGNet and main-model launchers share exp/.background state so that they do
not intentionally start two tracked GPU training jobs at the same time.
EOF
}
...
start_job() {
    ...
    if [[ ! -f "${EEG_NET_ENTRYPOINT}" ]]; then
        echo "Error: EEGNet entrypoint was not found: ${EEG_NET_ENTRYPOINT}" >&2
        exit 1
    fi
    ...
    local -a command=(
        ...
        python -u "${EEG_NET_ENTRYPOINT}"
    )
    if [[ "${mode}" == "eeg_net_sanity" ]]; then
    ...
    if ! kill -0 "${pid}" 2>/dev/null; then
        echo "Error: the EEGNet background process exited during startup." >&2
        tail -n 80 "${log_file}" >&2 || true
        exit 1
    fi

    echo "Background EEGNet ${mode} job started successfully."
    echo "PID: ${pid}"
    echo "Launcher log: ${log_file}"
    echo "The SSH connection can now be closed safely."
    echo "Status: ${PROJECT_ROOT}/run_eeg_net_background.sh status"
    echo "Logs:   ${PROJECT_ROOT}/run_eeg_net_background.sh tail"
}
...
case "${action}" in
    start)
        start_job "eeg_net_train" "$@"
        ;;
    sanity)
        start_job "eeg_net_sanity" "$@"
        ;;
    status)
        show_status
        ;;
